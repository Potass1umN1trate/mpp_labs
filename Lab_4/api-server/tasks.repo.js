...
const qListAll = db.prepare(`select id, title, status, due_date from tasks where user_id = ? order by id desc`)
const qListByStat = db.prepare(`select id, title, status, due_date FROM tasks  WHERE user_id = ? and status = ? order by id DESC`)
const qGetOwned = db.prepare(`SELECT id, title, status, due_date FROM tasks where id = ? and user_id = ?`)
const qUpdateTask = db.prepare(`UPDATE tasks SET title = COALESCE(?, title), status = COALESCE(?, status), due_date = COALESCE(?, due_date) WHERE id = ? AND user_id = ?`)
